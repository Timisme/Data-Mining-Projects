...
class SimRank_matrix():
	...
	def get_SimRank(self, C= 0.6, epsilon= 1e-4, epochs= 20):
		...
		for _ in range(epochs):
			...
			old_S = S 
			S = C*np.dot(np.dot(self.Q, old_S), self.Q.transpose()) + (1-C) * I 
...
def main():
	...
	Ranker = SimRank_matrix(filename= filename)
	print('-'*50)
	print(f'Rank:\n\n{Ranker.get_SimRank().round(2)}')
	print(len(Ranker.get_SimRank().round(2)))
