...
class Bert_BiLstm_Crf(nn.Module):
	...
	def _forward_alg(self, feats):
		...
		log_alpha = torch.Tensor(batch_size, 1, self.n_tags).fill_(-10000.).to(self.device)  #[batch_size, 1, 16]
	...
	def _get_lstm_features(self, input_ids, attention_mask):
	    ...
	    embeds = self._bert_enc(input_ids, attention_mask)  # [8, 75, 768]
	    # 过lstm
	...
	def forward(self, input_ids, attention_mask):  # dont confuse this with _forward_alg above.
		...
		lstm_feats = self._get_lstm_features(input_ids, attention_mask)  # [8, 180,768]
